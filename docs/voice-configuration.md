...
| `barge_in.min_ms` | `FLUXER_VOICE_BARGE_IN_MIN_MS` | `180` direct / `1200` auto-join | Voiced duration required before generic RMS interruption; uses `barge_in.window_ms` when set. | Lower for faster generic speech interruption; raise to avoid speaker echo. Stop phrases use the separate semantic fast path below. |
| `barge_in.window_ms` | `FLUXER_VOICE_BARGE_IN_WINDOW_MS` | `0` | Sliding window for accumulating bursty voice activity; `0` means continuous speech only. | Set around `1000-1200` for speaker/soundbar setups where real speech blinks on/off but short echo bursts should not stop playback. |
...
| `barge_in.stop_phrase_energy_threshold` | `FLUXER_VOICE_BARGE_IN_STOP_PHRASE_ENERGY_THRESHOLD` | `450` | Lower RMS threshold for STT-verified stop commands while assistant audio is playing. | Keep below generic barge threshold so quiet/long “stop” can still be semantically verified. |
| `barge_in.stop_phrase_min_ms` | `FLUXER_VOICE_BARGE_IN_STOP_PHRASE_MIN_MS` | `120` | Minimum candidate speech before running STT stop-command verification. | Lower is faster but may STT more echo/noise. |
| `barge_in.stop_phrase_silence_ms` | `FLUXER_VOICE_BARGE_IN_STOP_PHRASE_SILENCE_MS` | `180` | Silence used to finalize the stop-command candidate. | Lower is faster; raise if stop words are clipped. |
| `barge_in.stop_phrase_max_seconds` | `FLUXER_VOICE_BARGE_IN_STOP_PHRASE_MAX_SECONDS` | `2.0` | Maximum candidate audio sent to STT for stop-command verification. | Keep small for interruption latency. |
