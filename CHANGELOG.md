...
- Reduced STT-backed loop latency by defaulting to `tiny.en` local STT, shortening the reliable fixed capture window to 3s, and streaming text-grounded xAI voice deltas directly into LiveKit instead of waiting for a full WAV before publishing; live first-audio timing improved to about 8.5s after capture start, with VAD capture kept experimental after it cut one live turn to 0.54s and produced empty STT.
