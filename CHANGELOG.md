...
- Hardened LiveKit smoke playback, xAI force-message generation, the legacy xAI room loop, nested barge-in config bridging, Python 3.10 timeout handling, and timeout/cleanup API consistency after review: one-shot audio tracks are now unpublished/stopped after playout, forced text-to-speech requests explicitly trigger `response.create`, remote stream end exits cleanly, final partial speech segments are preserved, YAML `voice.barge_in.*` settings reach both the gateway and child supervisor, asyncio timeouts are caught/suppressed compatibly across supported Python versions, public WAV and sink APIs wrap timeouts consistently, and failed publisher setup is closed defensively.
